...
module lexer =
  ...
  let private keywords = ["cin"; 
    ...
    "if"; 
    "int"; 
    "real"; // adding real literal to keywords
    "main"; 
    "true";
    "void"
  ]
  ...
  let rec private lexer nextc input tokens = 
      ...
      |  _  when List.contains nextc ['0'..'9'] ->
               ...
               let (literal,lookahead) = collectIntLiteral (nextChar input) input (string nextc)
                // right side of decimal is real_literal and ahead is pointing to whats after the right side
               if lookahead = '.' then
                let (right_literal, lookahead) = collectIntLiteral (nextChar input) input ("")
                lexer lookahead input (("real_literal:" + (literal + "." + right_literal)) :: tokens)
